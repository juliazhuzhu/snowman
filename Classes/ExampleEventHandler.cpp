...
void ExampleEventHandler::onItemPurchased(CCPurchasableVirtualItem *purchasableVirtualItem, cocos2d::__String *payload) {
    ...
    CCSoomlaUtils::logDebug(TAG,purchasableVirtualItem->getItemId()->getCString());
    int balance = soomla::CCStoreInventory::sharedStoreInventory()->getItemBalance(purchasableVirtualItem->getItemId()->getCString());
    if (strcmp("15_DIAMOND_ID", purchasableVirtualItem->getItemId()->getCString()) == 0)
