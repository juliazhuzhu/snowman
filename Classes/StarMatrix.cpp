...
void StarMatrix::onTouch(const Point& p){
	...
	if(s){
        if (m_placeBomb)
            genBombSeletedList(s);
        else if (m_replaceStar)
        {
            //if (m_layer->getState() == GameLayer::READY)
                m_layer->replaceStar(s);
            return;
        }
        else
            genSelectedList(s);
...
void StarMatrix::genBombSeletedList(Star* s){
    selectedList.clear();
    ...
    if ((index_j -1 >= 0) && ((index_i + 1) < ROW_NUM) )
    {
        star = stars[index_i + 1][index_j - 1];
        if(star && !star->isSelected()){
    ...
    if ( (index_j + 1 < COL_NUM) && (index_i -1 >=0))
    {
        star = stars[index_i - 1][index_j + 1];
        if(star && !star->isSelected()){
            star->setSelected(true);
    ...
    if ((index_i + 1 < ROW_NUM) && (index_j + 1 < COL_NUM))
    {
        star = stars[index_i + 1][index_j + 1];
        if(star && !star->isSelected()){
            star->setSelected(true);
...
void StarMatrix::deleteSelectedList(){
	...
	//refreshScore();
	m_layer->showLinkNum(start_pos,selectedList.size());
	adjustMatrix();
	if(isEnded()){
...
void StarMatrix::replaceStar()
{
    m_replaceStar = true;
}

void StarMatrix::compleateReplaceStar()
{
    m_replaceStar = false;
}
