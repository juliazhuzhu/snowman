...
void showStarParticleEffect(int color,Point position,Node* node){
    ...
    node->addChild(circleSprite, 10);
    circleSprite->setPosition(position);
    circleSprite->setScale(0);// start size
    circleSprite->runAction(Sequence::create(ScaleTo::create(0.3f, 1.0),
                                             CallFunc::create(CC_CALLBACK_0(Sprite::removeFromParent, circleSprite)),
                                             NULL));
    
    // 3. particle effect
    auto particleStars = ParticleSystemQuad::create("splash.plist");
    particleStars->setAutoRemoveOnFinish(true);
    particleStars->setBlendAdditive(false);
    particleStars->setPosition(position);
    particleStars->setScale(0.6);
    node->addChild(particleStars, 20);
